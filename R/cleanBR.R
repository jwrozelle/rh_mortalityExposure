...
BRdata <- BRdata.mw10.df

# BR Data recode

cleanBR <- function(BRdata) {
  ...
  if(BRdata$v000[1] == "MM7") {
    BRdata$c_dob_dd <- 15
  } else if (BRdata$v000[1] == "MW5") {
    BRdata$c_dob_dd <- 15
  }
  ...
  # date of birth in R format
  BRdata$c_dobR <- NA
  BRdata$c_dobR <- as.Date(paste(BRdata$c_dob_yyyy, BRdata$c_dob_mm, BRdata$c_dob_dd, sep = "-"))
  ...
  BRdata = BRdata %>%
    dplyr::arrange(caseid, bord) %>%
    dplyr::mutate(ID = dplyr::row_number()) %>%
    dplyr::mutate(fe_pre_sex = 0) %>%
    dplyr::mutate(fe_pre_surv = 0)
  ...
  # death in specific age ranges
  ##  28 days to <12 months
  BRdata$death_28dTo11m <- NA
  BRdata$death_28dTo11m <- ifelse(BRdata$b7 < 12 & BRdata$neonatal_death != 1, 1, 0)
  
  ##  12 months to < 60 months
  BRdata$death_12mTo59m <- NA
  BRdata$death_12mTo59m <- ifelse(BRdata$b7 >= 12 & BRdata$b7 < 60, 1, 0)
  ...
  BRdata_looper.df$prec6m_anyDeath <- 0
  BRdata_looper.df$prec6m_neoDeath <- 0
  BRdata_looper.df$prec6m_infDeath <- 0
  BRdata_looper.df$prec6m_u5Death <- 0
  
  # f_id <- unique(BRdata_looper.df$femaleID)[1]
  
  
  PrecedingPredictors <- lapply(unique(BRdata_looper.df$femaleID), function(f_id) {
    
    # get the subset for each female ID
    BR_f_sub <- filter(BRdata_looper.df, femaleID == f_id)
    
    # sort by birth order # added for R&R
    BR_f_sub <- BR_f_sub[order(BR_f_sub$bord),]
    
    # first facility based delivery # added for R&R
    BR_f_sub$first_fbd <- (NA)
    
    
    if (nrow(BR_f_sub) > 1 ) {  # added for R&R
      firstBirth.df <- filter(BR_f_sub, bord == 1)  # added for R&R
      BR_f_sub$first_fbd[2:nrow(BR_f_sub)] <- ifelse(rep(firstBirth.df$rh_del_fbd, nrow(BR_f_sub)-1) == 1, TRUE, FALSE) # added for R&R
    }
    
    BR_f_sub$first_fbd <- as.logical(BR_f_sub$first_fbd) # added for R&R
    ...
    if (sum(BR_f_sub$alive == 0) > 0) { 
      ...
      for(birth in 2:nrow(BR_f_sub)) {
        ...
        preceding_deaths.df <- filter(BR_f_sub, do_death < BR_f_sub$c_dobR[birth])
        # We also want deaths > 6 months before child's birth
        preceding_6m_deaths.df <- filter(BR_f_sub, BR_f_sub$c_dobR[birth] - do_death >= (30.437*6))
        ...
        # Deaths 6 months before birth
        # preceding neonatal deaths
        if(sum(preceding_6m_deaths.df$neonatal_death, na.rm = T) > 0) {
          BR_f_sub$prec6m_neoDeath[birth] <- 1
        }
        # preceding infant deaths
        if(sum(preceding_6m_deaths.df$infant_death, na.rm = T) > 0) {
          BR_f_sub$prec6m_infDeath[birth] <- 1
        }
        
        # preceding u5 deaths
        if(sum(preceding_6m_deaths.df$U5_death, na.rm = T) > 0) {
          BR_f_sub$prec6m_u5Death[birth] <- 1
        }
