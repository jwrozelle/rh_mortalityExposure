...
cleanKR_vaxx <- function(KRdata) {
  ...
  if (KRvac$v000[1] == "MW7") {
    KRvac <- KRvac %>%
      ...
      # mutate(rotav3 = case_when(h59%in%c(1,2,3) ~ 1, h59%in%c(0,8) ~ 0  )) %>% # Malawi does not collect rota3
      mutate(rotavsum= rotav1+
               rotav2# +
               # rotav3
      )
  ...
  # All basic vaccinations in
  KRvac$vaxx_allBasic_12to23 <- NA
  if (KRvac$v000[1] == "MW7") {
    KRvac$vaxx_allBasic_12to23 <- ifelse(
      KRvac$ch_bcg_either == 1 & 
        KRvac$dpt3 == 1 &
        KRvac$ch_polio3_either == 1 &
        KRvac$ch_pneumo3_either == 1 & 
        KRvac$ch_rotav2_either == 1 & 
        KRvac$ch_meas_either == 1
        , 
      1, 0)
      
  }
  
  KRvac <- preMergeTrim(KRdata, KRvac, by = "childID")
  KRdata <- merge(KRdata, KRvac, all.x = T, all.y=F, by = "childID")
