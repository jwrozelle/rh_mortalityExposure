...
## Table X [No longer included]

Parity specific model results for sister's exposure

### FBD Models

```{r sisMort_table3_models}
...
# model formula!!
fbd_matExp.f <- formula(paste0(
  "rh_del_fbd ~ ",
  paste(fbd_matExp.iVars, collapse = " + ")
)
)

(pairs(filter(inputData, bord == 1)[,fbd_matExp.iVars]))
  
## Parity 1
fm_fbd_prevSis_par1.model <- lme4::glmer(
  update(fbd_matExp.f, reformulate(c(".", "(1|v001)"))), 
  family=binomial, 
  data=filter(inputData, bord == 1), 
...
## Parity 2
fm_fbd_prevSis_par2.model <- lme4::glmer(
  update(fbd_matExp.f, reformulate(c(".", "(1|v001)"))),
  family=binomial, 
...
# fm_fbd_prevSis_par2.allFit <- allFit(fm_fbd_prevSis_par2.model)



# ### Without random intercept
# fm_fbd_prevSis_par2.model <- glm(
#   rh_del_fbd ~ # confirmed to be children aged 0 - 59 months
#     prev_sd_del_b4Dur_ever +
#     # bord +
#     wealthIndex +
...
# )
# 
# summary(fm_fbd_prevSis_par2.model)

## Parity 3+ ####
### With random intercept
fm_fbd_prevSis_par3p.model <- lme4::glmer(
  update(fbd_matExp.f, reformulate(c(".", "bord + (1|v001)"))),
  family=binomial, 
...
### Without random intercept
fm_fbd_prevSis_par3p.model <- glm(
  rh_del_fbd ~ # confirmed to be children aged 0 - 59 months
    prev_sd_del_b4Dur_ever +
    bord +
    wealthIndex +
...
### Parity 4+
fm_fbd_prevSis_par4p.model <- glm(
  rh_del_fbd ~ # confirmed to be children aged 0 - 59 months
    prev_sd_del_b4Dur_ever +
    bord +
    wealthIndex +
...
summary(fm_fbd_prevSis_par4p.model)

summary(fm_fbd_prevSis_par1.model)
summary(fm_fbd_prevSis_par2.model) # seems to be okay - all optimizers converge around the same place.
summary(fm_fbd_prevSis_par3p.model)

save(fm_fbd_prevSis_par1.model, file = file.path(getwd(), "data", "fm_fbd_prevSis_par1.Rdata"))
...
# fit the model
## Parity 1
fm_anc_prevSis_par1.model <- lme4::glmer(
  rh_anc_4vs ~ 
    prev_sd_del_b4Dur_ever +
    # bord +
    wealthIndex + 
...
## Parity 2
fm_anc_prevSis_par2.model <- lme4::glmer(
  rh_anc_4vs ~
    prev_sd_del_b4Dur_ever +
    # bord +
    wealthIndex +
...
# )
# 
# summary(fm_anc_prevSis_par2.model)

fm_anc_prevSis_par2.allFit <- allFit(fm_anc_prevSis_par2.model, parallel = "multicore")
ss2 <- summary(fm_anc_prevSis_par2.allFit)
ss2$which.OK


## Parity 3+
fm_anc_prevSis_par3p.model <- lme4::glmer(
  rh_anc_4vs ~ 
    prev_sd_del_b4Dur_ever +
    bord +
    wealthIndex + 
...
# Summaries ####
summary(fm_anc_prevSis_par1.model)
summary(fm_anc_prevSis_par2.model)
summary(fm_anc_prevSis_par3p.model)
...
models_facility_based_delivery <- list(fm_fbd_prevSis_par1.model, 
                                       fm_fbd_prevSis_par2.model, 
                                       fm_fbd_prevSis_par3p.model)
models_antenatal_care <- list(fm_anc_prevSis_par1.model, 
                              fm_anc_prevSis_par2.model, 
                              fm_anc_prevSis_par3p.model)

# Create a function to extract only the desired predictor and add stars
...
# Extract coefficients, SE, and stars for both outcomes
coef_se_star_fbd <- do.call("rbind", lapply(models_facility_based_delivery, extract_predictor, "prev_sd_del_b4Dur_everTRUE"))
coef_se_star_anc <- do.call("rbind", lapply(models_antenatal_care, extract_predictor, "prev_sd_del_b4Dur_everTRUE"))

# Combine coefficients, SE, and stars
combined_table <- cbind(coef_se_star_fbd, coef_se_star_anc)

# Create LaTeX table manually
cat("\n\\begin{table}[!htbp] \n")
cat("\\centering \n")
cat("\\caption{Coefficients and SE for prev\\_sd\\_del\\_b4Dur\\_ever} \n")
cat("\\begin{tabular}{l c c c c} \n")
cat("\\hline\\hline \n")
cat("& \\multicolumn{2}{c}{Facility Based Delivery} & \\multicolumn{2}{c}{Antenatal Care} \\\\ \n")
cat("& Coefficient & (SE) & Coefficient & (SE) \\\\ \n")
cat("\\hline \n")
cat("Parity 1 &", combined_table[1, 1], "& (", combined_table[1, 2], ") &", combined_table[1, 3], "& (", combined_table[1, 4], ") \\\\ \n", sep = "")
cat("Parity 2 &", combined_table[2, 1], "& (", combined_table[2, 2], ") &", combined_table[2, 3], "& (", combined_table[2, 4], ") \\\\ \n", sep = "")
cat("Parity 3+$^{\\dag}$ &", combined_table[3, 1], "& (", combined_table[3, 2], ") &", combined_table[3, 3], "& (", combined_table[3, 4], ") \\\\ \n", sep = "")
cat("\\hline \n")
cat("\\multicolumn{5}{l}{\\scriptsize{$^{***}$p<0.001$; $^{**}$p<0.01; $^{*}$p<0.05$}} \\\\ \n")
cat("\\multicolumn{5}{l}{\\scriptsize{$^{\\dag}$ Also adjusts for parity}} \\\\ \n")
cat("\\end{tabular} \n")
cat("\\end{table} \n")
...
cmortFullModels.list <- list(
  ...
  fm_anc_prevInf_recent.model,
  fm_anc_prevu5_recent.model,
  fm_fbd_prevNeo_recent.model,
  fm_fbd_prevInf_recent.model,
  fm_fbd_prevu5_recent.model
  )
...
# texreg(cmortFullModels.list,
#        caption = "Log odds of attending full antenatal care and facility-based delivery after exposure to own child, infant, or neonatal death",
#        caption.above = T,
#        label = "table:chExp_full",
...
#        )
# 
# htmlreg(cmortFullModels.list,
#         file = file.path("data", "table2.doc"),
#        caption = "Multilevel logistic regression models of the association between a woman’s experience of a previous child’s death and maternal health service use (log odds).",
#        caption.above = T,
#        label = "table:chExp_full",
#        override.coef = exp(coef(logit)),
#        custom.coef.map = list(
#          "prec_neoDeathTRUE" = "Neonatal death exposure",
#          "prec_neoDeath" = "Neonatal death exposure",
#          "prec_infDeath" = "Infant death exposure",
...
## Table S1 - Parity specific descriptives

### Create datasets

```{r}

IRdata_SecondChild_bord2 <- filter(
  IRdata_desc, 
  bord == 2 & 
    b0 < 2 &
    # bidx == 1 &
...
BRdata_SecondChild_bord2 <- filter(
  BRdata,
  bord == 2 &
    b0 < 2 &
    # bidx == 1 &
...
IRdata_SecondChild_bord3p <- filter(
  IRdata_desc, 
  bord >= 3 & 
    b0 < 2 &
    # bidx == 1 &
...
BRdata_SecondChild_bord3p <- filter(
  BRdata,
  bord >= 3 &
    b0 < 2 &
    # bidx == 1 &
...
BRdata_bord3p_recent.list <- future_lapply(unique(BRdata_SecondChild_bord3p$femaleID), function(femaleID_n) {
  BRdata.femaleID <- dplyr::filter(BRdata_SecondChild_bord3p, femaleID == femaleID_n)
  
  if (nrow(BRdata_bord3p_recent) >1) {
    BRdata.femaleID <- dplyr::filter(BRdata.femaleID, bidx == 1)
  }
  
  return(BRdata.femaleID)
}, future.seed = T)

BRdata_bord3p_recent.df <- iotools::fdrbind(BRdata_bord3p_recent.list)

parityRestricted.list <- list(
  anc_prevChild2 = IRdata_SecondChild_bord2,
  fbd_prevChild2 = BRdata_SecondChild_bord2,
  anc_prevChild3p = IRdata_SecondChild_bord3p,
  fbd_prevChild3p = BRdata_SecondChild_bord3p
)
...
# exposure to previous under 5 death
prec_u5DeathSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "prec_u5Death", useNA=F, roundby=1)
})
...
# exposure to previous infant death
prec_infDeathSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "prec_infDeath", useNA=F, roundby=1)
})
...
# exposure to previous neonatal death
prec_neoDeathSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "prec_neoDeath", useNA=F, roundby=1)
})
...
# rural
ruralSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "rural", useNA=F, roundby=1)
})
...
# everUnion
everUnionSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "everUnion", useNA=F, roundby=1)
})
...
# wealth Index
wealthIndexSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "wealthIndex", useNA=F, roundby=1)
})
...
# education Level
edLevelSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "edLevel", useNA=F, roundby=1)
})
...
# age group
atCBirth_5grpSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "atCBirth_5grp", useNA=F, roundby=1)
})
...
# exposure to sister maternal mortality
rh_anc_4vsSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "rh_anc_4vs", useNA=F, roundby=1)
})
...
# exposure to sister maternal mortality
rh_del_fbdSum.list <- lapply(parityRestricted.list, function(x) {
  # df <- subset(x, everUnion ==0)
  cat_sum(x, "rh_del_fbd", useNA=F, roundby=1)
})
...
for (i in rownames(table1_cat.df)) {
  table1_cat.df[i, "Total Respondents"] <- nrow(parityRestricted.list[[i]])
...
saveWorkbook(table1, file=file.path(getwd(), "data", "tableUpdated_ps.xlsx"), overwrite = T)

```

## Table S2 Effect of own-child deaths independent of neo-natal deaths
...
texreg(list(
  ...
  fm_anc_nonNeoU5_bord2.model, 
  fm_fbd_deathCat_bord2.model,
  fm_fbd_nonNeoU5_bord2.model),
  exp(coef()),
  caption = "Models assessing the effect of post-neonatal deaths on attending full antenatal care and facility-based delivery",
  caption.above = T,
  label = "tab:childNoNeo",
  custom.coef.map = list(
...
htmlreg(list(
  ...
  fm_fbd_deathCat_bord2.model,
  fm_fbd_nonNeoU5_bord2.model),
  file = file.path("data", "table_s2.doc"),
  caption = "Models assessing the effect of post-neonatal deaths on attending full antenatal care and facility-based delivery",
  caption.above = T,
  label = "tab:childNoNeo",
  custom.coef.map = list(
...
## Table s3 - Log odds of full antenatal care and facility-based delivery for models restricted to parity 2
...
texreg(list(
  ...
  fm_fbd_prevu5_bord2.model
       ...
       caption.above = T,
       caption="Log odds of full antenatal care and facility-based delivery for models restricted to parity 2",
       label = "table:childExp_bord2",
       custom.coef.map = list(
         "prec_neoDeath" = "Neonatal death exposure",
         "prec_neoDeathTRUE" = "Neonatal death exposure",
         "prec_infDeath" = "Infant death exposure",
...
htmlreg(list(
  ...
  file = file.path("data", "table_s3.doc"),
       caption.above = T,
       caption="Log odds of full antenatal care and facility-based delivery for models restricted to parity 2",
       label = "table:childExp_bord2",
       custom.coef.map = list(
...
## Table s4 - Log odds of full antenatal care and facility-based delivery for models restricted to parity 3 or higher

```{r}
load(file.path("data", "fbd_otherParityModels.Rdata"))
load(file.path("data", "anc_otherParityModels.Rdata"))

texreg(list(fm_anc_prevu5_bord3p.model, 
            fm_anc_prevInf_bord3p.model, 
            fm_anc_prevNeo_bord3p.model, 
            fm_fbd_prevu5_bord3p.model, 
            fm_fbd_prevInf_bord3p.model, 
       ...
       caption.above = T,
       caption="Log odds of full antenatal care and facility-based delivery for models restricted to parity 3 or higher",
       label = "table:childExp_bord3p",
       custom.coef.map = list(
         "prec_u5DeathTRUE" = "Under five death exposure",
         "prec_infDeath" = "Infant death exposure",
         "prec_infDeathTRUE" = "Infant death exposure",
...
htmlreg(list(
  ...
  file = file.path("data", "table_s4.doc"),
       caption.above = T,
       caption="Log odds of full antenatal care and facility-based delivery for models restricted to parity 2",
       label = "table:childExp_bord3p",
       custom.coef.map = list(
...
cMortModels.list <- list(fm_anc_prevu5_bord3p.model, 
            fm_anc_prevInf_bord3p.model, 
            fm_anc_prevNeo_bord3p.model, 
            fm_fbd_prevu5_bord3p.model, 
            fm_fbd_prevInf_bord3p.model, 
...
## Table S5 - Robustness models using sister's death unrelated to pregnancy or delivery
...
texreg(list(fm_anc_prevSis_rb.model, fm_fbd_prevSis_rb.model),
       caption = "Robustness check for associaiton between sister's non-maternal death for exposures on attending full antenatal care (Model 1) or facility-based delivery (Model 2)",
       custom.coef.map = list(
         "(Intercept)" = "(Intercept)",
         "prev_sd_del_b4Dur_ever" = "Sister's Death during pregnancy / delivery",
         "prev_sd_nonMat_ever" = "Sister's nonmaternal death",
...
htmlreg(list(fm_anc_prevSis_rb.model, fm_fbd_prevSis_rb.model),
        file = file.path("data", "table_s5.doc"),
       caption = "Robustness check for associaiton between sister's non-maternal death for exposures on attending full antenatal care (Model 1) or facility-based delivery (Model 2)",
       custom.coef.map = list(
...
tab_model(
  ...
  dv.labels = c(
    ...
    "Model 3:\nFull antenatal care CHILD", 
    "Model 4:\nFacility-based delivery CHILD"
    ),
  p.style = "stars",
  pred.labels = pred.names,
  show.intercept = F,
  show.r2 = F,
  show.reflvl = T#,
  # file = file.path("data", "table_S2_NEW.doc")
  )
...
tab_model(
  fm_fbd_prevNeo_bord2_recent.model,
  # dv.labels = c(
  #   "Model 1:\nFull antenatal care SIS", 
  #   "Model 2:\nFacility-based delivery SIS",
  #   "Model 3:\nFull antenatal care CHILD", 
  ...
  show.r2 = F,
  show.reflvl = T,
  file = file.path("data", "table_4c4_RECENT.doc")
  )
...
tab_model(
  ...
  show.r2 = F,
  show.reflvl = T,
  file = file.path("data", "table_S2_ADDITIONAL.doc")
  )
